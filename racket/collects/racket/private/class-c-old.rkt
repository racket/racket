...
(define (class/c-check-first-order ctc cls fail)
  ...
  (for ([m (in-list (class/c-methods ctc))]
    ...
    (unless mth (fail '("no public method ~a" given: "~e") m cls))
    (when c
      (define meth-proc
...
(define (internal-class/c-check-first-order internal-ctc cls fail)
  ...
  (for ([m (internal-class/c-inherits internal-ctc)])
    (unless (hash-ref method-ht m #f)
      (fail '("no public method ~a" given: "~e") m cls)))
    ...
    (let ([index (hash-ref method-ht m #f)])
      ...
      (let ([vec (vector-ref beta-methods index)])
        (unless (zero? (vector-length vec))
          (fail '("method ~a was previously augmentable" given: "~e") m cls)))
      (let ([flag (vector-ref meth-flags index)])
        (when (eq? flag 'final)
          (fail '("method ~a is final" given: "~e") m cls)))))
    ...
    (let ([index (hash-ref method-ht m #f)])
      ...
      (let* ([vec (vector-ref beta-methods index)])
        (when (zero? (vector-length vec))
          (fail '("method ~a has never been augmentable" given: "~e") m cls))
        (when (vector-ref vec (sub1 (vector-length vec)))
          (fail '("method ~a is currently overrideable, not augmentable" given: "~e") m cls)))))
    ...
    (let ([index (hash-ref method-ht m #f)])
      ...
      (let ([vec (vector-ref beta-methods index)])
        ...
        (unless (vector-ref vec (sub1 (vector-length vec)))
          (fail '("method ~a is currently augmentable, not overrideable" given: "~e") m cls)))))
  (for ([s (internal-class/c-supers internal-ctc)])
    (let ([index (hash-ref method-ht s #f)])
      (unless index
        (fail '("no public method ~a" given: "~e") s cls))
      (let ([flag (vector-ref meth-flags index)])
        (when (eq? flag 'final)
          (fail '("method ~a is final" given: "~e") s cls))
        (when (eq? flag 'augmentable)
          (fail '("method ~a is augmentable, not overrideable" given: "~e") s cls)))))
  (for ([i (internal-class/c-inners internal-ctc)])
    (let ([index (hash-ref method-ht i #f)])
      (unless index
        (fail '("no public method ~a" given: "~e") i cls))
      (let ([vec (vector-ref beta-methods index)])
        (when (zero? (vector-length vec))
          (fail '("method ~a has never been augmentable" given: "~e") i cls)))
      (let ([flag (vector-ref meth-flags index)])
        (when (eq? flag 'final)
          (fail '("method ~a is final" given: "~e") i cls)))))
  (define field-ht (class-field-ht cls))
      ...
      (define graph-edges-from-val
        (cond
          ...
          [(hash-ref val-to-edge-hash-and-ctc val #f)
           =>
           (λ (pair)
             (define ctc-with-graph-edges (car pair))
             (define graph-edges-from-val (cdr pair))
             (and (contract-equivalent? ctc ctc-with-graph-edges)
                  graph-edges-from-val))]
          [else #f]))
      ...
      (define existing-wrapper
        (cond
          ...
          [(null? contracts-already-on-object)
           (cond
             [(hash-ref graph-edges-from-val (blame-add-missing-party blame neg-party) #f)
              => values]
             [else
              ;(printf "fail; no edge with matching blame; value has no contract\n")
              #;
              (printf "blames: ~s ~s ~s ~s\n"
                      (eq-hash-code val) val
                      (list (blame-negative (blame-add-missing-party blame neg-party))
                            (blame-positive (blame-add-missing-party blame neg-party)))
                      (for/list ([b (hash-keys graph-edges-from-val)])
                        (list (blame-negative b)
                              (blame-positive b))))
              #f])]
          [(and (pair? contracts-already-on-object)
                (null? (cdr contracts-already-on-object)))
           ...
           [else
            ...
            (when new-graph-edges
              ...
              (when (< (hash-count val-to-edge-hash-and-ctc) 20)
                (hash-set! val-to-edge-hash-and-ctc val (cons ctc new-graph-edges))))
