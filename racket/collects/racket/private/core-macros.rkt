...
#;(define-syntaxes (and or)
  (let-values ()
  
    (define-values (do-and-or-transformer)
      (lambda (is-and?)
        (define-values (tmp-name)
          (if is-and? (quote-syntax and-part) (quote-syntax or-part)))
        
        (lambda (stx)
          ; Decompose stx into `(me . args)`
          (define-values (lst) (syntax->list stx))
          (if (pair? lst)
              (void)
              (raise-argument-error #f "bad syntax" lst))
          (define-values (me)
            (if (identifier? (car lst))
                (car lst)
                (if is-and? 'and 'or)))
          (define-values (args) (cdr lst))
          (letrec-values ([(loop)
                           (lambda (args)
                             (if (null? args)
                                 (void)
                                 (if (keyword? (syntax-e (car args)))
                                     (raise-syntax-error me "bad syntax (expected an expression)" (car args))
                                     (loop (cdr args)))))])
            (loop args))
          ...
          (datum->syntax #f
                         (if (null? args)
                             (if is-and?
                                 ...
                                 (quote-syntax '#f))
                             (if (null? (cdr args))
                                 (list (quote-syntax #%expression) (car args))  ; annoying special case
                                 (letrec-values ([(loop)
                                                  (lambda (remaining-reversed-args)
                                                    (define-values (z rest) (car+cdr remaining-reversed-args))
                                                    (if (null? rest)
                                                        ; (or z)  => z
                                                        ; (and z) => z
                                                        z
                                                        ; (or a ... y z)  => (or  a ... (let ([y y]) (if y y z)))
                                                        ; (and a ... y z) => (and a ... (let ([y y]) (if y z #f)))
                                                        (letrec-values ([(y rest2) (car+cdr rest)])
                                                          (loop (cons (list (quote-syntax let-values)
                                                                            (list (list (list tmp-name) y))
                                                                            (if is-and?
                                                                                (list (quote-syntax if) tmp-name z (quote-syntax '#f))
                                                                                (list (quote-syntax if) tmp-name tmp-name z)))
                                                                      rest2)))))])
                                   (loop (alt-reverse args)))))
                         (quote-syntax here)))))

    (values (do-and-or-transformer #t)
            (do-and-or-transformer #f))))
...
(define-syntaxes (define-values-for-syntax)
  (lambda (stx)
    (define-values (lst) (syntax->list stx))
    (if (if lst (= 3 (length lst)) #f)
        (void)
        (raise-syntax-error #f "bad syntax" stx))
    (define-values (ids) (syntax->list (cadr lst)))
    (if (if ids (andmap identifier? ids) #f)
        (void)
