...
(#%require (for-syntax '#%kernel "stx.rkt" "pico.rkt")
           (for-meta 2 '#%kernel "stx.rkt" "pico.rkt"))

(#%provide and or
           cond old-cond else =>
           let let* letrec
           let*-values ; let-values and letrec-values are core forms
           quasiquote
           when unless
           define  ; define-values is a core form
           define-syntax  ; define-syntaxes is a core form
...
(define-syntaxes (begin-for-phases)
  (lambda (stx)
    ; Decompose invocation into parts
    ;   (begin-for-phases #:phases  (ph ...)   body ...*)
    ;    ^-0 car          ^-1 cadr  ^-2 caddr  ^-cdddr-^
    (define-values (lst) (syntax->list stx))
    (if lst #t (raise-syntax-error #f "bad syntax" stx))
    (if (null? lst) (raise-syntax-error #f "bad syntax" stx) #t)
    (if (null? (cdr lst)) (raise-syntax-error #f "bad syntax (missing #:phases)" stx) #t)
    (if (null? (cddr lst)) (raise-syntax-error #f "bad syntax (missing list of phases)" stx (cadr lst)) #t)
    (define-values (me) (car lst))
    (define-values (phases-kw-stx) (cadr lst))
    (define-values (phase-lst-stx) (caddr lst))
    (define-values (orig-body) (cdddr lst))
    ; Validate phases list and phases kw
    (if (eq? '#:phases (syntax->datum phases-kw-stx))
        #t
        (raise-syntax-error #f "bad syntax (expected '#:phases' here)" stx phases-kw-stx))
    (define-values (phase-lst) (syntax->datum phase-lst-stx))
    (let-values ()
      (define-values (check-phase-list-loop)
        (lambda (prev remaining)
          (if (null? remaining)
              #t
              (if (exact-nonnegative-integer? (car remaining))
                  (if (prev . < . (car remaining))
                      (check-phase-list-loop (car remaining) (cdr remaining))
                      #f)
                  #f))))
      (if (if phase-lst (check-phase-list-loop -1 phase-lst) #f)
          #t
          (raise-syntax-error #f "bad syntax (expected increasing list of phase levels)" stx phase-lst)))
    ; Construct form which is body at a specific phase
    (define-values (phaseless-body)
      (syntax-track-origin (datum->syntax #f
                                          (list* (quote-syntax begin) orig-body)
                                          stx)
                           stx
                           me))
    (define-values (wrap-form)
      (lambda (form n)
        (if (zero? n)
            form
            (wrap-form
             (syntax-track-origin (datum->syntax #f
                                                 (list (quote-syntax begin-for-syntax) form)
                                                 form)
                                  form
                                  me)
             (sub1 n)))))
    (define-values (output-form-list)
      (map (lambda (ph) (wrap-form phaseless-body ph)) phase-lst))
    ; Finish
    (datum->syntax #f
                   (list* (quote-syntax begin) output-form-list)
                   stx)))
...
(begin-for-phases
  ...
  (define-syntaxes (and)
    (let-values ([(here) (quote-syntax here)])
      (lambda (x)
        (if (not (stx-list? x))
            (raise-syntax-error #f "bad syntax (not a list of expressions)" x)
            (void))
...
(define-syntaxes (when)
  (lambda (stx)
    ...
    (if (null? (cdr lst))
        (raise-syntax-error #f "bad syntax (missing test expression and body)" stx)
        (void))
    (if (null? (cddr lst))
...
(define-syntaxes (define-values-for-syntax)
  (lambda (stx)
    (define-values (lst) (syntax->list stx))
    (or (and lst (= 3 (length lst)))
        (raise-syntax-error #f "bad syntax" stx))
    (define-values (ids) (syntax->list (cadr lst)))
    (or (and ids (andmap identifier? ids))
        (raise-syntax-error #f "bad syntax (not a list of identifiers)" stx (cadr lst)))
    (datum->syntax #f
...
(begin-for-syntax
  (define-values (process-function-define)
    (lambda (full-stx head body-list)  ; invariant: head != identifier
      (or (stx-pair? head)
          (raise-syntax-error #f "bad syntax (illegal function head)" full-stx head))
      (define-values (inner-head) (stx-car head))
      (define-values (arg-stxlist) (stx-cdr head))
      (define-values (this-layer-rhs)
        (datum->syntax #f
                       (list* (quote-syntax lambda) arg-stxlist body-list)
                       full-stx))
      (if (identifier? inner-head)
          (values inner-head this-layer-rhs)
          (process-function-define full-stx inner-head (list this-layer-rhs)))))
  
  (define-values (process-define)
    (lambda (output-form-head stx)
      (define-values (lst) (syntax->list stx))
      (or (and lst (<= 3 (length lst)))
          (raise-syntax-error #f "bad syntax" stx))
      (define-values (me) (car lst))
      (define-values (outermost-head) (cadr lst))
      (define-values (body-lst) (cddr lst))
      (define-values (id rhs)
        (if (identifier? outermost-head)
            (begin
              (or (= 1 (length body-lst))
                  (raise-syntax-error #f
                                      "bad syntax (expected single value expr for non-function definition)"
                                      stx
                                      (cadddr lst)))
              (values outermost-head (car body-lst)))
            (process-function-define stx outermost-head body-lst)))
      (datum->syntax #f
                    (list output-form-head
                          (list id)
                          rhs)
                    stx))))
...
(define-syntaxes (let/ec)
  (lambda (stx)
    ...
    (if (= len 1)
        (raise-syntax-error #f "bad syntax (missing identifier)" stx)
        (void))
    (if (= len 2)
        (raise-syntax-error #f "bad syntax (missing body)" stx)
        (void))
