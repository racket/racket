...
(define-for-syntax build-unit-from-context
  (let ()
    (struct defn-info (ext-id loc-id ctc dup-safe? post-def?) #:transparent)
    (syntax-parser
      #:context (current-syntax-context)
       ...
       (define unit-expr
         (build-make-unit
          ...
          (λ ()
            ...
            (define/syntax-parse ([loc-id ...] [ctcd-loc-id ...] [body-id ...] [export-thunk ...])
              (for/fold ([loc-ids '()]
                         [ctcd-loc-ids '()]
                         [body-ids '()]
                         [export-thunks '()]
                         #:result (list (reverse loc-ids)
                ...
                (cond
                  ...
                  [else
                   ...
                   (bound-id-table-set! defns-table int-id (defn-info ext-id loc-id ctc dup-safe? post-def?))
                   (values (cons loc-id loc-ids)
                           (if ctc (cons loc-id ctcd-loc-ids) ctcd-loc-ids)
                           (if post-def? body-ids (cons int-id body-ids))
                  ...
                  (values
                   (with-loc+name #,(current-syntax-context)
                     (lambda (import-table)
                       ...
                       #,@(for/list ([body-id (in-list (attribute body-id))])
