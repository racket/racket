(module kernel '#%kernel
  ...
  (module reader '#%kernel
    (#%module-begin
     ...
     (define-values (get-the-lang) (lambda () (quote '#%kernel)))
     ...
     (define-values (wrap-internal)
       (lambda (stx? src port modpath line col pos)

         (define-values (maybe-tag)
           (lambda (v loc props)
             (if stx?
                 (datum->syntax #f v loc props)
                 v)))

         (define-values (read-next)
           (lambda ()
             (if stx?
         ...
         (define-values (read-loop)
           (lambda (accum)
             (let-values ([(v) (read-next)])
               (if (eof-object? v)
                   (reverse accum)
                   (read-loop (cons v accum))))))

         (define-values (all-body)
         ...
         (letrec-values ([(body) (maybe-wrap-module-begin all-body)]
                         [(name) (port->module-name port)])
           (maybe-tag (list (maybe-tag 'module   modpath-loc #f)
                            (maybe-tag name      modpath-loc #f)
                            (maybe-tag (get-the-lang) modpath     modpath)
                            body)
                      all-loc
                      #f))))
     ...
     (define-values (the-language-info)
       (lambda (what defval)
         (if (eq? what 'module-language) (get-the-lang) defval))))))
