...
(define (ssl-server-context-enable-ecdhe! context [name #f])
  ;; No longer necessary: since v1.1.0 ECDH support is always enabled, and curve
  ;; is automatically selected. Can customize with SSL_set1_groups[_list], but
  ;; generally unnecessary, so currently unsupported.
  ;; For v1.0.2, ECDHE enabled in auto mode in (server) context construction.
  (void))
