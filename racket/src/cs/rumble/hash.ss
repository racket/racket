...
(define-hash-constructors hashequalw make-immutable-hashequalw empty-hashequalw)
  ...
  (cond
   ...
   [(intmap? ht)
    (let ([new-ht (cond
                   ...
                   [(intmap-equal-always? ht) (make-hashequalw)]
                   [else (make-hash)])])
  ...
  (cond
   [(intmap? ht)
    (cond
     ...
     [(hash-equal-always? ht) empty-hashequalw]
     [else empty-hash])]
   [(and (impersonator? ht)
         (intmap? (impersonator-val ht)))
