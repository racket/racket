...
(module (np-expand-primitives)
  ...
  (with-output-language (L7 Expr)
    ...
    (let () ; level 3 char=?, r6rs:char=?, etc.
      ...
      (char-pred char>=? r6rs:char>=? >=)
      (char-pred char>? r6rs:char>? >))
    (define-inline 3 char-grapheme-cluster-step
      [(e-ch e-state)
       (bind #t (e-ch e-state)
         ...
         `(if ,(build-and
              ...
              (if ,(%inline eq? ,e-state (immediate ,(fix ($char-grapheme-cluster-other-state))))
                  ,(%primcall src sexpr values ,(%constant strue) (immediate ,(fix ($char-grapheme-cluster-other-state))))
                  (if ,(%inline eq? ,e-state (immediate ,(fix 0)))
                      ,(%primcall src sexpr values ,(%constant sfalse) (immediate ,(fix ($char-grapheme-cluster-other-state))))
                      ,(%primcall src sexpr $char-grapheme-cluster-step ,e-ch ,e-state)))
              ,(%primcall src sexpr $char-grapheme-cluster-step ,e-ch ,e-state)))])
    (define-inline 3 map
      [(e-proc e-ls)
