...
;
; Generally, we want to emit a warning in this case, as it's possible
; that we stumbled on an actual bug (e.g. the bug fixed in git commit
;     b9b017261b (io: repair wrong-variable use in checking for special Windows paths, 2025-06-03)
; However, there are also a handful of cases where the code is
; (obviously to humans) safe, and our analysis is too primitive to
; recognize it. What we do in this case is to act soundly and indicate
; a warning, *except* that we suppress the warning when the code looks
; like one of the cases that a human reviewed. (There's a more detailed
; discussion of why in GH PR https://github.com/racket/racket/pull/5272
; "extracter: rewrite, bugfix, and test mutated-vars".)
;
; If you've found your way to this comment because a warning showed up
; and you're investigating :) then you can (1) set `debug` to #t below;
; (2) regenerate the schemified files, which will generate the `.rktl`
; linklet files in the process; (3) use the debugging info to identify
; the relevant code in the linklet, and then trace that back to the
; `.rkt` source; and (4) either fix the bug, if there is one, or
; otherwise add a case to this match statement to find your code and
; suppress the warning. If possible, keep the match limited so that
; it doesn't suppress warnings of possibly-unsafe patterns unlike the
; one you found, and definitely don't do anything beyond supressing
; warnings because we want the analysis to remain sound.
(define (maybe-warn-about-suspect-letrec letrec-expr)
  (match letrec-expr
    [`(letrec-values ([(,name) (,some-event-ctor ,e1s ... (lambda ,_ ,e2) ,e3s ...)]) ,body)
     #:when (or (string-contains? (symbol->string some-event-ctor) "wrap-evt")
                (string-contains? (symbol->string some-event-ctor) "replacing-evt"))
     #:when (andmap (lambda (e)
                      (set-empty? (set-intersect (frees e) (seteq name))))
                    (append e1s e3s))
     (void)]

    [els
     (log-warning "WARN: unable to show that implicit assignment in letrec is unobservable")
     (log-warning "Details about this message and next steps are in the source code.")
     (let ([debug #f])
       (when debug
         (log-status "full letrec expression: ~a" letrec-expr)))]))
     ...
     (define names-because-implicit-assignments/or-false
       (for/or ([bound-expr (in-list bound-exprs)])
         (match bound-expr
           ...
           [els
            (maybe-warn-about-suspect-letrec e)
...
(module+ test
  ...
  (define (check-mutated-vars* expr expected [desc #f] #:warn [warn-rx #f] #:loc loc)
    (parameterize ([current-status-output-port (open-output-string)])
      (define actual-sorted (sort (set->list (mutated-vars expr)) symbol<?))
      (define expected-sorted (sort (set->list (list->seteq expected)) symbol<?))
      (define out-str (get-output-string (current-status-output-port)))
  ...
  (check-mutated-vars '(letrec-values ([(vvv) (proc vvv)])
                         body)
                      '(vvv)
                      #:warn #rx"WARN.*implicit assignment.*vvv"
  ...
  (check-mutated-vars '(letrec-values ([(vvv) (lambda () vvv)]
                         ...
                         body)
                      '(vvv zzz)
                      #:warn #rx"WARN.*implicit assignment.*(vvv.*zzz|zzz.*vvv)"
  ...
  (check-mutated-vars '(letrec-values ([(aaa) 4]  ; <-- inclusion in result is conservative
                         ...
                         body)
                      '(aaa bbb ccc ddd eee)
                      #:warn #rx"WARN.*implicit assignment"
