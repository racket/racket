...
@racket[_dep-path-or-target] list, but a [:target] line with multiple
@racket[_path]s creates a single target that builds all of the
@racket[_path]s. A @racket[:db-dir] line (appearing at most once)
specifies where build information should be recorded for all targets;
otherwise the build result for each target is stored in the target's
directory.
