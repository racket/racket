...
static Scheme_Object *tcp_copy_file(int argc, Scheme_Object *argv[]);
...
void scheme_init_network(Scheme_Startup_Env *env)
{
  ...
  ADD_PRIM_W_ARITY2 ( "tcp-copy-file"             , tcp_copy_file            , 2 , 4 , 2 , 2 , env) ;
...
static Scheme_Object *tcp_copy_file(int argc, Scheme_Object *argv[]) {
  ...
  intptr_t nbytes = -1;
  if (!SCHEME_OUTPUT_PORTP(argv[0])) {
    scheme_wrong_contract("tcp-copy-file", "(and/c tcp-port? output-port?)", 0, argc, argv);
    return NULL;
  }
  ...
  if (op->closed) {
    scheme_raise_exn(MZEXN_FAIL_NETWORK, "tcp-copy-file: tcp port closed");
    return NULL;
  }

  if (!SCHEME_PATH_STRINGP(argv[1])) {
    scheme_wrong_contract("tcp-copy-file", "path-string?", 1, argc, argv);
    return NULL;
  }
