...
typedef struct Scheme_Mark {
  ...
  Scheme_Hash_Table *owner_multi_mark;
} Scheme_Mark;
...
XFORM_NONGCING int scheme_mark_subset(Scheme_Mark_Set *sa, Scheme_Mark_Set *sb)
{
...
Scheme_Object *extract_single_mark(Scheme_Object *multi_mark, Scheme_Object *phase)
{
  ...
  if (!m) {
    m = scheme_new_mark();
    ((Scheme_Mark *)m)->owner_multi_mark = ht;
...
static Scheme_Mark_Set *extract_mark_set(Scheme_Stx *stx, Scheme_Object *phase)
{
  ...
  if (SAME_OBJ(phase, scheme_make_integer(0)))
    marks = stx->marks->phase_0;
  else if (SAME_OBJ(phase, scheme_make_integer(1)))
    marks = stx->marks->phase_1;
  else {
    marks = (Scheme_Mark_Set *)scheme_hash_tree_get(stx->marks->other_phases,
                                                    phase);
    if (!marks)
      marks = empty_mark_set;
  }

  for (multi_marks = stx->marks->multi_marks;
                                           ...
                                           do_mark_t do_mark, do_mark_list_t do_mark_list, Scheme_Mark_Table *prev)
{
  ...
  if (SCHEME_MARKP(m) && ((Scheme_Mark *)m)->owner_multi_mark)
    m = (Scheme_Object *)((Scheme_Mark *)m)->owner_multi_mark;
...
{
  if (SCHEME_MARKP(mark) || SCHEME_MULTI_MARKP(mark))
...
static Scheme_Object *shift_multi_mark(Scheme_Object *p, Scheme_Object *shift)
{
  ...
  if (SCHEME_FALSEP(shift)) {
    if (!SAME_OBJ(phase, scheme_make_integer(0)))
      ...
      return scheme_make_pair(SCHEME_CAR(p), scheme_false);
  } else if (SCHEME_FALSEP(phase))
    return p;
  else
    return scheme_make_pair(SCHEME_CAR(p), scheme_bin_plus(phase, shift));
}

static Scheme_Object *shift_prop_multi_mark(Scheme_Object *p, Scheme_Object *shift)
{
  Scheme_Object *phase, *p2;
  
  phase = SCHEME_VEC_ELS(p)[1];
  if (SCHEME_FALSEP(shift)) {
    if (!SAME_OBJ(phase, scheme_make_integer(0)))
      return NULL;
    else
                                           ...
                                           shift_multi_mark_t shift_mm, Scheme_Mark_Table *prev)
{
  ...
  if (prev) {
    if (SCHEME_FALSEP(shift)
        || SCHEME_FALSEP(((Scheme_Propagate_Table *)mt)->phase_shift))
      val = scheme_false;
    else
...
static void print_marks(Scheme_Mark_Set *marks, Scheme_Mark_Set *check_against_marks)
{
  ...
  while (i != -1) {
    ...
    if (((Scheme_Mark *)key)->owner_multi_mark)
      printf("/%" PRIdPTR, scheme_hash_key((Scheme_Object *)((Scheme_Mark *)key)->owner_multi_mark));
...
void scheme_add_binding_copy(Scheme_Object *o, Scheme_Object *from_o, Scheme_Object *phase)
{
  ...
  add_binding(stx->val, phase, extract_mark_set(stx, phase), result);
}
...
Scheme_Object *scheme_make_top_level_module_context(Scheme_Object *phase)
{
  ...
  vec = scheme_make_vector(5, NULL);
  SCHEME_VEC_ELS(vec)[0] = top_level_multi_marks;
  SCHEME_VEC_ELS(vec)[1] = phase;
  SCHEME_VEC_ELS(vec)[2] = scheme_false;
  SCHEME_VEC_ELS(vec)[3] = scheme_null;
  SCHEME_VEC_ELS(vec)[4] = SCHEME_CAR(SCHEME_CDR(top_level_multi_marks));
...
static void unmarshal_module_context_additions(Scheme_Stx *stx, Scheme_Object *vec, Scheme_Mark_Set *marks)
{
  Scheme_Object *modidx, *unmarshal_info, *context, *src_phase;
  Scheme_Hash_Table *export_registry;
  ...
  scheme_do_module_context_unmarshal(modidx, context, src_phase,
...
static Scheme_Object *intern_tails(Scheme_Object *l, Scheme_Hash_Table *ht)
{
  ...
  while (!SCHEME_NULLP(r)) {
    ...
    scheme_hash_set(ht, result, result);
    r = SCHEME_CDR(r);
  }
...
static Scheme_Object *marks_to_sorted_list(Scheme_Mark_Set *marks)
{
  ...
  a = MALLOC_N(Scheme_Object *, mark_set_count(marks));
...
static Scheme_Object *wraps_to_datum(Scheme_Stx *stx, Scheme_Marshal_Tables *mt)
{
  ...
  v = intern_tails(drop_export_registries(stx->shifts), ht);
  SCHEME_VEC_ELS(vec)[0] = v;

  v = intern_tails(marks_to_sorted_list(stx->marks->phase_0), ht);
  SCHEME_VEC_ELS(vec)[1] = v;

  v = intern_tails(marks_to_sorted_list(stx->marks->phase_1), ht);
  ...
  while (i != -1) {
    ...
    v = scheme_make_pair(scheme_make_pair(key, val), v);
    i = scheme_hash_tree_next(stx->marks->other_phases, i);
  }
  sort_list(v, compare_phase_at_car);
  v = intern_tails(v, ht);
...
Scheme_Mark_Set *list_to_mark_set(Scheme_Object *l, Scheme_Unmarshal_Tables *ut)
{
  ...
  while (!SCHEME_NULLP(r)) {
    l = SCHEME_CAR(r);
    if (!SCHEME_MARKP(SCHEME_CAR(l))) return_NULL;

    marks = mark_set_set(marks, SCHEME_CAR(l), scheme_true);
                                     ...
                                     Scheme_Unmarshal_Tables *ut)
{
  ...
  mt->phase_0 = marks;
  STX_ASSERT(SCHEME_MARK_SETP(marks));
...
void scheme_mark_unmarshal_content(Scheme_Object *m, Scheme_Object *c, Scheme_Unmarshal_Tables *ut)
{
  Scheme_Object *l = scheme_null, *l2, *r;
  Scheme_Hash_Table *ht;
  Scheme_Mark_Set *marks;
  intptr_t i, len;
  ...
  if (SCHEME_VECTORP(c)) {
    ...
    for (i = 0; i < len; i += 2) {
      ...
      while (SCHEME_PAIRP(l2)) {
        ...
        if (marks)
          r = scheme_make_pair(scheme_make_pair((Scheme_Object *)marks,
                                                SCHEME_CDR(SCHEME_CAR(l2))),
                               r);
        l2 = SCHEME_CDR(l2);
      }
    ...
    l = scheme_make_pair((Scheme_Object *)scheme_make_hash_table(SCHEME_hash_ptr), l);
...
Scheme_Object *scheme_syntax_make_transfer_intro(int argc, Scheme_Object **argv)
{
  ...
  if (!SCHEME_FALSEP(src)) {
    ...
    if (!scheme_mark_subset(m2, delta))
      m2 = NULL;
  } else
    m2 = NULL;
