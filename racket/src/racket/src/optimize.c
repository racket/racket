...
struct Optimize_Info
{
  ...
  Scheme_Compiled_Local *transitive_use_var; /* set when optimizing a letrec-bound procedure */
...
static Scheme_Object *optimize_info_lookup(Optimize_Info *info, Scheme_Object *var, int closure_ok, int *single_use,
                                           int once_used_ok, int context, int *potential_size, int *_mutated);
static void optimize_info_used_top(Optimize_Info *info);
static Scheme_Object *do_optimize_get_predicate(Optimize_Info *info, Scheme_Object *var, int ignore_no_types);
...
static int optimize_any_uses(Optimize_Info *info, Scheme_Compiled_Let_Value *at_clv, int n);
static void propagate_used_variables(Optimize_Info *info);
static int env_uses_toplevel(Optimize_Info *frame);
static Scheme_Compiled_Local *clone_variable(Scheme_Compiled_Local *var);
static void increment_use_count(Scheme_Compiled_Local *var, int as_rator);
...
static Scheme_Object *optimize_clone(int dup_ok, Scheme_Object *obj, Optimize_Info *info, Scheme_Hash_Tree *var_map, int as_rator);
...
static Scheme_Once_Used *make_once_used(Scheme_Object *val, Scheme_Compiled_Local *var,
...
static void set_optimize_mode(Scheme_Compiled_Local *var)
{
        ...
        instead of omitted; opt_info must be provided to check mutability. */
{
 ...
 try_again:
  ...
  if ((vtype > _scheme_compiled_values_types_)
      || ((vtype == scheme_compiled_local_type)
          && (no_id <= 0)
  ...
  if (vtype == scheme_set_bang_type) {
    ...
    if (SAME_TYPE(scheme_local_type, SCHEME_TYPE(sb->var))
      ...
      return 1;
    else if (SAME_TYPE(scheme_compiled_local_type, SCHEME_TYPE(sb->var))
...
static int is_local_ref(Scheme_Object *e, int p, int r, Scheme_Compiled_Local **vars)
{
  if (!vars && SAME_TYPE(SCHEME_TYPE(e), scheme_local_type)) {
    if ((SCHEME_LOCAL_POS(e) >= p)
        && (SCHEME_LOCAL_POS(e) < (p + r)))
      return 1;
  } else if (vars && SAME_TYPE(SCHEME_TYPE(e), scheme_compiled_local_type)) {
    int i;
    for (i = p; i < p + r; i++) {
...
static int ok_proc_creator_args(Scheme_Object *rator, Scheme_Object *rand1, Scheme_Object *rand2, Scheme_Object *rand3,
                                int delta2, int field_count, Scheme_Compiled_Local **vars)
{
                                    ...
                                    int context, Scheme_Object *orig, Scheme_Object *le_prev)
{
  ...
  Scheme_Compiled_Local **vars;
  Scheme_Closure_Data *data = (Scheme_Closure_Data *)p;
  ...
  for (i = 0; i < expected; i++) {
    ...
    vars = MALLOC_N(Scheme_Compiled_Local*, 1);
    vars[0] = ((Closure_Info *)data->closure_map)->vars[i];
    lv->vars = vars;
  ...
  if (le && SAME_TYPE(SCHEME_TYPE(le), scheme_compiled_unclosed_procedure_type) && (info->inline_fuel >= 0)) {
    ...
    *_flags = SCHEME_CLOSURE_DATA_FLAGS(data);
	...
	/* If optimize_clone succeeds, inlining succeeds. */
        le = optimize_clone(single_use, (Scheme_Object *)data, sub_info, empty_eq_hash_tree, 0);
                                          ...
                                          Optimize_Info *info)
{
  ...
  if (SAME_TYPE(SCHEME_TYPE(rator), scheme_compiled_local_type)) {
    {
      int single_use;
      le = optimize_info_lookup(info, rator, 1, &single_use, 0, 0, NULL, NULL);
   ...
   pattern again in optimize_for_inline() after optimizing a rator. */
{
  ...
  if (SAME_TYPE(SCHEME_TYPE(rator), scheme_compiled_local_type)) {
    Scheme_Compiled_Local *var = SCHEME_VAR(rator);
  ...
  case scheme_compiled_local_type:
    {
      if (!SCHEME_VAR(expr)->mutated) {
        ...
        if (info) {
          p = optimize_get_predicate(info, expr);
          if (p)
            return p;
        }
                                           ...
                                           Optimize_Info *info)
{
  if (SAME_OBJ(rator, scheme_call_with_immed_mark_proc)
    ...
    e = (Scheme_Object *)((Closure_Info *)((Scheme_Closure_Data *)rand2)->closure_map)->vars[0];
    e = scheme_make_mutable_pair(e, ((Scheme_Closure_Data *)rand2)->code);
    wcm->body = e;
   ...
   If unsafe is NULL then rator has no unsafe version, so only check the type. */
{
  if (SCHEME_PRIMP(rator) && IS_NAMED_PRIM(rator, who)) {
    ...
    if (pred) {
      ...
      if (SAME_TYPE(SCHEME_TYPE(rand), scheme_compiled_local_type)) {
...
static Scheme_Object *lookup_constant_proc(Optimize_Info *info, Scheme_Object *rand)
{
  ...
  if (SAME_TYPE(scheme_compiled_unclosed_procedure_type, SCHEME_TYPE(rand)))
    c = rand;
  else if (SAME_TYPE(SCHEME_TYPE(rand), scheme_compiled_local_type))
    c = optimize_info_lookup(info, rand, 1, NULL, 0, 0, NULL, NULL);
...
static Scheme_Object *finish_optimize_application2(Scheme_App2_Rec *app, Optimize_Info *info, int context, int rator_flags)
{
  ...
  if ((SAME_OBJ(scheme_values_func, rator)
      ...
      && ((context & OPT_CONTEXT_SINGLED)
          || scheme_omittable_expr(rand, 1, -1, 0, info, info, 0, 0, ID_OMIT)
...
static Scheme_Object *flatten_sequence(Scheme_Object *o, Optimize_Info *info, int context)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((SAME_TYPE(SCHEME_TYPE(o3), scheme_sequence_type) && !(!i && b0))
      ...
      extra += s3->count;
      split++;
    } else if (SAME_TYPE(SCHEME_TYPE(o3), scheme_compiled_let_void_type) && !(!i && b0)) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((SAME_TYPE(SCHEME_TYPE(o3), scheme_sequence_type) && !(!i && b0))
      ...
      /* move rest under `let`: */
      Scheme_Let_Header *head = (Scheme_Let_Header *)o3;
...
int scheme_compiled_duplicate_ok(Scheme_Object *fb, int cross_module)
{
  return (SCHEME_VOIDP(fb)
	  ...
	  || SCHEME_INTP(fb)
	  || SCHEME_NULLP(fb)
	  || (!cross_module && SAME_TYPE(SCHEME_TYPE(fb), scheme_compiled_local_type))
          || SCHEME_PRIMP(fb)
...
/* pos is in new-frame counts */
{
  if (!SCHEME_VAR(var)->mutated) {
    ...
    pred = optimize_get_predicate(info, var);
    if (pred) {
                                       ...
                                       Optimize_Info *a_info, Optimize_Info *b_info, int context)
{
  ...
  if (SAME_TYPE(SCHEME_TYPE(a), scheme_compiled_local_type)
      && SAME_OBJ(a, b))
    return a;

  if (b_info 
      && SAME_TYPE(SCHEME_TYPE(a), scheme_compiled_local_type)
  ...
  if (a_info 
      && SAME_TYPE(SCHEME_TYPE(b), scheme_compiled_local_type)
...
static void add_type(Optimize_Info *info, Scheme_Object *var, Scheme_Object *pred)
{
  ...
  if (do_optimize_get_predicate(info, var, 1)
      || SCHEME_VAR(var)->val_type) {
    return;
  }
...
static void add_types_for_t_branch(Scheme_Object *t, Optimize_Info *info, int fuel)
{
  ...
  if (SAME_TYPE(SCHEME_TYPE(t), scheme_application2_type)) {
    ...
    if (SCHEME_PRIMP(app->rator)
        && SAME_TYPE(SCHEME_TYPE(app->rand), scheme_compiled_local_type)
    ...
    Scheme_Object *pred1, *pred2;
    if (SAME_OBJ(app->rator, scheme_eq_prim)) {
      if (SAME_TYPE(SCHEME_TYPE(app->rand1), scheme_compiled_local_type)
          && !SCHEME_VAR(app->rand1)->mutated) {
        pred1 = optimize_get_predicate(info, app->rand1);
        if (!pred1) {
          pred2 = expr_implies_predicate(app->rand2, info, NULL, 5);
          if (pred2)
      ...
      if (SAME_TYPE(SCHEME_TYPE(app->rand2), scheme_compiled_local_type)
          && !SCHEME_VAR(app->rand2)->mutated) {
        pred2 = optimize_get_predicate(info, app->rand2);
        if (!pred2) {
          pred1 = expr_implies_predicate(app->rand1, info, NULL, 5);
          if (pred1)
...
static void add_types_for_f_branch(Scheme_Object *t, Optimize_Info *info, int fuel)
{
  ...
  if (SAME_TYPE(SCHEME_TYPE(t), scheme_compiled_local_type)) {
    add_type(info, t, scheme_not_prim);
...
static Scheme_Object *
set_clone(int dup_ok, Scheme_Object *data, Optimize_Info *info, Scheme_Hash_Tree *var_map)
{
  ...
  val = optimize_clone(dup_ok, val, info, var_map, 0);
  if (!val) return NULL;
  if (SAME_TYPE(SCHEME_TYPE(var), scheme_compiled_local_type)) {
    var = optimize_clone(dup_ok, var, info, var_map, 0);
    if (!var) return NULL;
  }
...
static Scheme_Object *
ref_clone(int dup_ok, Scheme_Object *data, Optimize_Info *info, Scheme_Hash_Tree *var_map)
{
  Scheme_Object *naya;
  Scheme_Object *a, *b;

  a = SCHEME_PTR1_VAL(data);
  a = optimize_clone(dup_ok, a, info, var_map, 0);
  if (!a) return NULL;

  b = SCHEME_PTR2_VAL(data);
  b = optimize_clone(dup_ok, b, info, var_map, 0);
  if (!b) return NULL;
...
static Scheme_Object *
apply_values_clone(int dup_ok, Scheme_Object *data, Optimize_Info *info, Scheme_Hash_Tree *var_map)
{
  Scheme_Object *f, *e;
  ...
  f = optimize_clone(dup_ok, f, info, var_map, 0);
  if (!f) return NULL;
  e = optimize_clone(dup_ok, e, info, var_map, 0);
  if (!e) return NULL;
...
static Scheme_Object *
with_immed_mark_clone(int dup_ok, Scheme_Object *data, Optimize_Info *info, Scheme_Hash_Tree *var_map)
{
  Scheme_With_Continuation_Mark *wcm = (Scheme_With_Continuation_Mark *)data;
  ...
  e = optimize_clone(dup_ok, wcm->key, info, var_map, 0);
  if (!e) return NULL;
  wcm2->key = e;

  e = optimize_clone(dup_ok, wcm->val, info, var_map, 0);
  if (!e) return NULL;
  wcm2->val = e;
  ...
  e = optimize_clone(dup_ok, SCHEME_CDR(wcm->body), info, var_map, 0);
  if (!e) return NULL;
...
static Scheme_Object *
case_lambda_clone(int dup_ok, Scheme_Object *data, Optimize_Info *info, Scheme_Hash_Tree *var_map)
{
  Scheme_Object *le;
  int i, sz;
  ...
  for (i = 0; i < seq->count; i++) {
    le = seq->array[i];
    le = optimize_clone(dup_ok, le, info, var_map, 0);
    if (!le) return NULL;
    seq2->array[i] = le;
  }
...
static int is_liftable_prim(Scheme_Object *v, int or_escape)
{
  ...
  if (SAME_OBJ(v, scheme_values_func))
    return 1;
  ...
  case scheme_compiled_let_void_type:
    {
      ...
      for (i = lh->num_clauses; i--; ) {
        if (!scheme_is_liftable(((Scheme_Compiled_Let_Value *)o)->value, exclude_vars, fuel - 1, as_rator, or_escape))
          return 0;
...
static int is_values_apply(Scheme_Object *e, int n, Optimize_Info *info, Scheme_Hash_Tree *except_vars, int fuel)
{
  if (SAME_TYPE(SCHEME_TYPE(e), scheme_application_type)) {
    ...
    if (SAME_TYPE(SCHEME_TYPE(b->test), scheme_compiled_local_type)
        && !scheme_hash_tree_get(except_vars, b->test)
        && scheme_omittable_expr(b->test, 1, -1, 0, info, info, 0, 0, NO_MUTABLE_ID_OMIT)) {
...
static void update_rhs_value(Scheme_Compiled_Let_Value *naya, Scheme_Object *e,
                             Optimize_Info *info, Scheme_Compiled_Local *tst)
{
  if (tst) {
    Scheme_Object *n;
...
static void unpack_values_application(Scheme_Object *e, Scheme_Compiled_Let_Value *naya,
                                      Optimize_Info *info, Scheme_Compiled_Local *branch_test)
{
  if (SAME_TYPE(SCHEME_TYPE(e), scheme_application_type)) {
    Scheme_App_Rec *app = (Scheme_App_Rec *)e;
    int i;
    ...
    MZ_ASSERT(SAME_TYPE(SCHEME_TYPE(b->test), scheme_compiled_local_type));
...
static int worth_lifting(Scheme_Object *v)
{
  ...
  if ((lhs == scheme_compiled_unclosed_procedure_type)
      ...
      || (lhs == scheme_compiled_local_type)
...
static void start_transitive_use_record(Optimize_Info *to_info, Optimize_Info *info, Scheme_Compiled_Local *var)
{
  if (var->optimize_used)
    return;
  
  info->transitive_use_var = var;
  var->optimize.transitive_uses_to = to_info;
...
{
  Scheme_Compiled_Local *var = info->transitive_use_var;
...
scheme_optimize_lets(Scheme_Object *form, Optimize_Info *info, int for_inline, int context)
{
  ...
  int did_set_value, unused_clauses, found_escapes;
  ...
  if (context & OPT_CONTEXT_BOOLEAN) {
    ...
    if (!(SCHEME_LET_FLAGS(head) & SCHEME_LET_RECURSIVE) && (head->count == 1) && (head->num_clauses == 1)) {
      ...
      if (SAME_TYPE(SCHEME_TYPE(clv->body), scheme_branch_type)
          && (clv->vars[0]->use_count == 2)) {
        Scheme_Branch_Rec *b = (Scheme_Branch_Rec *)clv->body;
        if (SAME_OBJ(b->test, (Scheme_Object *)clv->vars[0])
            && SAME_OBJ(b->tbranch, (Scheme_Object *)clv->vars[0])) {
          Scheme_Branch_Rec *b3;
  ...
  if (!(SCHEME_LET_FLAGS(head) & SCHEME_LET_RECURSIVE) && (head->count == 1) && (head->num_clauses == 1)) {
    clv = (Scheme_Compiled_Let_Value *)head->body;
    if (SAME_OBJ((Scheme_Object *)clv->vars[0], clv->body)) {
  ...
  for (i = head->num_clauses; i--; ) {
    ...
    if ((pre_body->count == 1) && !pre_body->vars[0]->mutated) {
      ...
      if (value && SAME_TYPE(SCHEME_TYPE(value), scheme_compiled_local_type)) {
      ...
      if (value && (scheme_compiled_propagate_ok(value, body_info))) {
        ...
        if (SAME_TYPE(SCHEME_TYPE(value), scheme_compiled_local_type)) {
	...
	&& ((i < 1)
		...
		&& !scheme_is_liftable(((Scheme_Compiled_Let_Value *)pre_body->body)->value, merge_skip_vars, 5, 1, 0)))) {
         ...
         value = clv->value;
          ...
          if (cl_first && SAME_OBJ(value, SCHEME_CAR(cl_first))) {
            ...
            if ((clv->count == 1)
              ...
              start_transitive_use_record(body_info, rhs_info, clv->vars[0]);
            ...
            if (!clv->vars[0]->mutated) {
              if (scheme_compiled_propagate_ok(value, rhs_info)) {
                ...
                if (OPT_DELAY_GROUP_PROPAGATE || OPT_LIMIT_FUNCTION_RESIZE)
                  ...
                  new_sz = 0;
                if (new_sz <= sz) {
                  clv->vars[0]->optimize.known_val = value;
                }
                else if (!OPT_LIMIT_FUNCTION_RESIZE
                         || (new_sz < 4 * sz))
                  prop_later = scheme_make_raw_pair(scheme_make_pair((Scheme_Object *)clv->vars[0],
                                                                     value),
                                                    prop_later);
              }
            }
	    ...
	    break;
          {
            ...
            int i;
            for (i = clv->count; i--; ) {
              if (!clv->vars[i]->mutated) {
                SCHEME_VAR(SCHEME_CAR(ready_pairs_start))->optimize_unready = 1;
  ...
  if (!is_rec && (head->count == 1) && (head->num_clauses == 1)) {
    clv = (Scheme_Compiled_Let_Value *)head->body;
    if (SAME_OBJ(clv->body, (Scheme_Object *)clv->vars[0])) {
      if (worth_lifting(clv->value))
        return clv->value;
    }
  }
...
optimize_closure_compilation(Scheme_Object *_data, Optimize_Info *info, int context)
{
  ...
  data->closure_size = (cl->base_closure->count
			+ (cl->has_tl ? 1 : 0));
...
static Scheme_Compiled_Local *clone_variable(Scheme_Compiled_Local *var)
{
  Scheme_Compiled_Local *var2;
  MZ_ASSERT(SAME_TYPE(var->so.type, scheme_compiled_local_type));
  var2 = MALLOC_ONE_TAGGED(Scheme_Compiled_Local);
  memcpy(var2, var, sizeof(Scheme_Compiled_Local));
  return var2;
}

static Scheme_Compiled_Local **clone_variable_array(Scheme_Compiled_Local **vars,
                                                    int sz,
                                                    Scheme_Hash_Tree **_var_map)
{
  Scheme_Compiled_Local **new_vars, *var;
  Scheme_Hash_Tree *var_map = *_var_map;
  int j;
  
  new_vars = MALLOC_N(Scheme_Compiled_Local*, sz);
  for (j = sz; j--; ) {
...
static Scheme_Object *clone_closure_compilation(int dup_ok, Scheme_Object *_data, Optimize_Info *info, Scheme_Hash_Tree *var_map)
{
  ...
  vars = clone_variable_array(cl->vars, data2->num_params, &var_map);
  cl->vars = vars;

  body = optimize_clone(dup_ok, data->code, info, var_map, 0);
  if (!body) return NULL;
    ...
    if ((lh->count == n) && (lh->num_clauses == n)
      ...
      if ((n == 2) && SAME_TYPE(SCHEME_TYPE(body), scheme_application3_type)) {
        ...
        if (SAME_OBJ(app->rator, scheme_values_func)
            ...
            && SAME_OBJ(app->rand2, (Scheme_Object *)((Scheme_Compiled_Let_Value *)lv->body)->vars[0])) {
          if (vars) {
        ...
        if (SAME_OBJ(app->args[0], scheme_values_func)) {
          for (i = 0; i < n; i++) {
            if (!SAME_TYPE(SCHEME_TYPE(app->args[i+1]), scheme_compiled_local_type)
          ...
          if (vars) {
            body = lh->body;
            for (i = 0; i < n; i++) {
              Scheme_Compiled_Let_Value *lv2 = (Scheme_Compiled_Let_Value *)body;
...
# include "mzstkchk.h"
  ...
  case scheme_compiled_local_type:
    {
      Scheme_Object *val;
      int is_mutated = 0, single_use;

      info->size += 1;

      val = optimize_info_lookup(info, expr, 0, &single_use,
...
Scheme_Object *optimize_clone(int dup_ok, Scheme_Object *expr, Optimize_Info *info, Scheme_Hash_Tree *var_map, int as_rator)
   ...
   any constant. */
{
  ...
  case scheme_application2_type:
    {
      ...
      expr = optimize_clone(dup_ok, app->rator, info, var_map, 1);
      if (!expr) return NULL;
      app2->rator = expr;

      expr = optimize_clone(dup_ok, app->rand, info, var_map, 0);
      if (!expr) return NULL;
      app2->rand = expr;
  ...
  case scheme_application_type:
    {
      ...
      for (i = app->num_args + 1; i--; ) {
	expr = optimize_clone(dup_ok, app->args[i], info, var_map, !i);
	if (!expr) return NULL;
	app2->args[i] = expr;
      }
  ...
  case scheme_application3_type:
    {
      ...
      expr = optimize_clone(dup_ok, app->rand1, info, var_map, 0);
      if (!expr) return NULL;
      app2->rand1 = expr;

      expr = optimize_clone(dup_ok, app->rand2, info, var_map, 0);
      if (!expr) return NULL;
      app2->rand2 = expr;
  ...
  case scheme_compiled_let_void_type:
	...
	head2->body = body;
      ...
      body = head2->body;
      for (i = head->num_clauses; i--; ) {
	lv2 = (Scheme_Compiled_Let_Value *)body;

        expr = optimize_clone(dup_ok, lv2->value, info, var_map, 0);
	if (!expr) return NULL;
	lv2->value = expr;
	...
	body = lv2->body;
      ...
      expr = optimize_clone(dup_ok, body, info, var_map, 0);
      if (!expr) return NULL;
  ...
  case scheme_splice_sequence_type:
    {
      ...
      for (i = seq->count; i--; ) {
	expr = optimize_clone(dup_ok, seq->array[i], info, var_map, 0);
	if (!expr) return NULL;
	seq2->array[i] = expr;
      }
  ...
  case scheme_branch_type:
    {
      ...
      expr = optimize_clone(dup_ok, b->test, info, var_map, 0);
      if (!expr) return NULL;
      b2->test = expr;

      expr = optimize_clone(dup_ok, b->tbranch, info, var_map, 0);
      if (!expr) return NULL;
      b2->tbranch = expr;

      expr = optimize_clone(dup_ok, b->fbranch, info, var_map, 0);
      if (!expr) return NULL;
      b2->fbranch = expr;
  ...
  case scheme_with_cont_mark_type:
    {
      ...
      expr = optimize_clone(dup_ok, wcm->key, info, var_map, 0);
      if (!expr) return NULL;
      wcm2->key = expr;

      expr = optimize_clone(dup_ok, wcm->val, info, var_map, 0);
      if (!expr) return NULL;
      wcm2->val = expr;

      expr = optimize_clone(dup_ok, wcm->body, info, var_map, 0);
      if (!expr) return NULL;
      wcm2->body = expr;
  ...
  case scheme_compiled_unclosed_procedure_type:
    return clone_closure_compilation(dup_ok, expr, info, var_map);
  case scheme_compiled_toplevel_type:
  case scheme_compiled_quote_syntax_type:
    return expr;
  case scheme_define_values_type:
  ...
  case scheme_require_form_type:
    return NULL;
  case scheme_varref_form_type:
    return ref_clone(dup_ok, expr, info, var_map);
  case scheme_set_bang_type:
    return set_clone(dup_ok, expr, info, var_map);
  case scheme_apply_values_type:
    return apply_values_clone(dup_ok, expr, info, var_map);
  case scheme_with_immed_mark_type:
    return with_immed_mark_clone(dup_ok, expr, info, var_map);
  case scheme_case_lambda_sequence_type:
    return case_lambda_clone(dup_ok, expr, info, var_map);
  case scheme_module_type:
    return NULL;
  default:
    if (t > _scheme_compiled_values_types_) {
...
static void register_transitive(Scheme_Compiled_Local *var);
static void register_use_at(Scheme_Compiled_Local *var, Optimize_Info *info);

static Scheme_Object *transitive_k(void)
{
  Scheme_Thread *p = scheme_current_thread;
  Scheme_Compiled_Local *var = SCHEME_VAR(p->ku.k.p1);

  p->ku.k.p1 = NULL;

  register_transitive(var);
...
# include "mzstkchk.h"
  ...
  for (j = 0; j < ht->size; j++) {
    if (ht->vals[j]) {
      tvar = SCHEME_VAR(ht->keys[j]);
      register_use_at(tvar, var->optimize.transitive_uses_to);
    }
  }
}
...
{
  int i, j;
  Scheme_Compiled_Let_Value *clv = at_clv;

  while (n--) {
    for (i = clv->count; i--; ) {
      if (clv->vars[i]->optimize_used)
        return 1;
      for (j = at_clv->count; j--; ) {
        if (at_clv->vars[j]->optimize.transitive_uses) {
          if (scheme_hash_get(at_clv->vars[j]->optimize.transitive_uses,
                              (Scheme_Object *)clv->vars[i]))
            return 1;
        }
      }
    }
  ...
  if (!var->optimize_used) {
    ...
    if (info->transitive_use_var
        && (var->optimize.lambda_depth
            <= info->transitive_use_var->optimize.transitive_uses_to->lambda_depth)) {
...
Scheme_Object *do_optimize_get_predicate(Optimize_Info *info, Scheme_Object *var, int ignore_no_types)
{
  Scheme_Object *pred;
