...
#define DISALLOW_EXTFLONUM(special, other)                      \
  if ((special && SCHEME_LONG_DBLP(special)) || (other && SCHEME_LONG_DBLP(other))) { \
    if (report)                                                         \
      return scheme_numr_err(complain, \
				  ...
				  int test_only)
{
  ...
  while (str[delta] == '#') {
	...
	if (complain)
	  return scheme_numr_err(complain,
                                 "bad exactness specification in `%u`", 
                                 str, len);
	else
	  return scheme_false;
      }
  ...
  if (!(len - delta)) {
    if (report)
      return scheme_numr_err(complain, "no digits");
    return scheme_false;
  }
...
#define isbaseNdigit(N, ch) (((ch >= 'a') && (ch <= (mzchar)('a' + N - 11))) \
  ...
  for (i = delta; i < len; i++) {
	...
	if (report)
	  return scheme_numr_err(complain,
                                 "cannot have slash at start in `%u`", 
                                 str, len);
	return scheme_false;
      }
      if (has_slash) {
...
#define MAX_FAST_FLOATREAD_LEN 50
  ...
  if (!is_not_float && (is_float || decimal_means_float)
    ...
    if (is_long_double && is_float)  {
      if (report)
        return scheme_numr_err(complain,
                               "cannot convert extflonum to inexact in `%u`",
                               str, len);
      return scheme_false;
    }
  ...
  if (has_decimal || has_expt || (has_hash && !has_slash)) {
    ...
    if (result_is_float) {
      ...
      if (is_long_double) {
        if (report)
          return scheme_numr_err(complain,
                                 "cannot convert extflonum to exact in `%u`",
                                 str, len);
        return scheme_false;
      }
