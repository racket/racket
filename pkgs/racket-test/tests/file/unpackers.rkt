...
(define (untar-tests*)
  ...
  (let ([out (open-output-bytes)])
    (parameterize ([current-output-port out])
      (tar->output (list (tar-entry 'directory "dir-from-string" #f 0 #hash()))))
    (define got (tar->entries (open-input-bytes (get-output-bytes out))))
    (unless (and (= 1 (length got))
                 (eq? 'directory (tar-entry-kind (car got)))
                 (equal? (string->path "dir-from-string")
                         (tar-entry-path (car got))))
      (error "tar-entry string path was not preserved correctly")))
  (make-directory* "sub")
