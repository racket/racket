...
When a string produced by the error value conversion handler
or in an unquoted-printing string contains a newline but does not
start with a newline, then the string is started on its own line with
extra spaces added before each line to indent the string content.
...
If the string produced by @racket[error-value->string-handler]
contains a newline but does not start with a newline, then a context
that uses the string will add spaces or indentation after each newline
as needed. For example, @racket[raise-argument-error] adds a
three-space indentation to the start of each line.
