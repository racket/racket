...
@tech{Parallel threads}, @tech{futures} and @tech{places} can expose the
underlying machine's memory model, including a weak memory ordering.
For example, when a parallel thread writes to multiple slots in a mutable
vector, it's possible on some platforms for another parallel thread to observe
...
Synchronization operations such as semaphores, @racket[sync], place messages, future
