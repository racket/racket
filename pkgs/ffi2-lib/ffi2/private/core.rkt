...
(define-for-syntax (parse-ffi2-cast stx unsafe?)
  (syntax-parse stx
    [(form-id expr:expr
     ...
     (cond
       [(and from-t (ffi2-type-scalar? from-t)
             t (ffi2-type-scalar? t))
        (when (attribute offset-expr)
          (raise-syntax-error #f "offset not allowed for scalar cast" stx #'offset-expr))
        #`(let ([val expr])
            #,@(ffi2-type-defns from-t)
            (unless (variable-reference-from-unsafe? (#%variable-reference))
              (unless (#,(ffi2-type-predicate from-t) val) (bad-cast-value 'form-id '#,(ffi2-type-name from-t) val)))
            (let ([c (#,(ffi2-type-racket->c from-t) val)])
