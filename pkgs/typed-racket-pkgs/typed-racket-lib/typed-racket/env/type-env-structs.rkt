...
;; dict is a free-id-table of identifiers to types
;; props is a list of known propositions
(define-struct/cond-contract env ([l immutable-free-id-table?] [props (listof Filter/c)])
  #:transparent
  #:property prop:custom-write
  (lambda (e prt mode)
    (fprintf prt "(env ~a ~a)" (free-id-table-map (env-l e) list) (env-props e))))
...
(define (lookup e key fail)
  (match e
    [(env l _) (free-id-table-ref l key (λ () (fail key)))]))
...
(define (extend/values e ks vs)
  (match e
    [(env l p)
     (env
       (for/fold ([l l]) ([k (in-list ks)] [v (in-list vs)])
         (free-id-table-set l k v))
       p)]))
