    ...
    (let loop ([ty ty] [typed-side (if typed-side 'typed 'untyped)] [structs-seen null] [kind kind])
           ...
           (cond
            ...
            [else
             ...
             (unless (no-duplicates (for/list ([t (in-list arrs)])
               ...
               (define reason
                 "function type has two cases of the same arity")
      ...
      (define (set-impersonator!)
        (when (not (equal? kind impersonator-sym))
          (exit (fail #:reason
                      (~a "required a chaperone or flat contract"
                          " but got an impersonator contract."))))
        (increase-current-contract-kind! impersonator-sym))
      (define (set-chaperone!)
        (when (equal? kind flat-sym)
          (exit (fail #:reason
                      (~a "required a flat contract but got"
                          " a chaperone contract."))))
        ...
        [(Struct: nm par (list (fld: flds acc-ids mut?) ...) proc poly? pred?)
         (cond
           [(assf (λ (t) (type-equal? t ty)) structs-seen)
            =>
            cdr]
           [proc (exit (fail #:reason "procedure structs are not supported"))]
           [(and (equal? kind flat-sym) (ormap values mut?))
            (exit (fail #:reason
                        (~a "expected a flat contract, but got"
                            " a mutable struct type")))]
           [poly?
