...
(Section 'free-id-set-as-set)
(require racket/set 
         (prefix-in 
          r: (only-in racket/set set-member? set-add set-remove))
         racket/syntax
         syntax/id-set)

;; maps other datums to identifiers
(define (as-id x) (format-id #'here "~a" x))
(define (set-equal? s) 
  (or (immutable-free-id-set? s)
      (mutable-free-id-set? s)))
(define set-eqv? set-equal?)
(define set-eq? set-equal?)
(define (set . xs) (immutable-free-id-set (map as-id xs)))
(define seteqv set)
(define seteq set)
(define set? immutable-free-id-set?)
(define (mutable-set . xs) (mutable-free-id-set (map as-id xs)))
(define mutable-seteqv mutable-set)
(define mutable-seteq mutable-set)
(define set-mutable? mutable-free-id-set?)
(define (set-member? s x) (r:set-member? s (as-id x)))
(define (set-add s x) (r:set-add s (as-id x)))
(define (set-remove s x) (r:set-remove s (as-id x)))
