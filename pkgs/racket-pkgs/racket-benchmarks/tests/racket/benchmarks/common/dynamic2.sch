...
(define (find! elem)
  ...
  ;(newline)
  (let ((p-elem (vector-ref elem 0)))
    (if (null? p-elem)
	elem
	(let ((rep-elem (find! p-elem)))
	  (vector-set! elem 0 rep-elem)
	  rep-elem))))
...
(define (link! elem-1 elem-2)
  ...
  ;(newline)
  (let ((rank-1 (vector-ref elem-1 1))
	(rank-2 (vector-ref elem-2 1)))
    (cond
     ((= rank-1 rank-2)
      (vector-set! elem-2 1 (+ rank-2 1))
      (vector-set! elem-1 0 elem-2)
      elem-2)
     ((> rank-1 rank-2)
      (vector-set! elem-2 0 elem-1)
      elem-1)
     (else
      (vector-set! elem-1 0 elem-2)
      elem-2))))

(define asymm-link! (lambda (l x) (vector-set! l 0 x)))
