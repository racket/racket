...
(let-values ([(prop:p p? p-ref) (make-struct-type-property 'prop (lambda (x y) (add1 x)))]
  ...
  (define-syntax-rule (make-struct-type-tests make-struct-type my-struct-type? make-arg ...)
    (let-values ([(type make pred sel set) (make-struct-type 'a #f 2 1 'un (list (cons prop:p 87)) (make-inspector insp1) make-arg ...)]
                 [(typex makex predx selx setx) (make-struct-type 'ax #f 0 5 #f null (make-inspector insp2) make-arg ...)])
      (cond
        [(null? '(make-arg ...))
         (arity-test make-struct-type 4 11)]
        [else
         (define n (+ 7 (length '(make-arg ...))))
         (arity-test make-struct-type n n)])
      (test #t struct-type? type)
      (test #t my-struct-type? type)
      ...
      (let ([sel1 (make-struct-field-accessor sel 0)]
        ...
        (let ([an-a (make 'one 'two)]
          ...
          (let-values ([(prop:p3 p3? p3-ref) (make-struct-type-property 'prop3)]
            ...
            (let-values ([(btype bmake bpred bsel bset) (make-struct-type 'b type 0 3 'unun (list (cons prop:p3 33) (cons prop:p4 44)) (make-inspector insp2) make-arg ...)]
                         [(btypex bmakex bpredx bselx bsetx) (make-struct-type 'bx typex 1 5 'nope (list (cons prop:p3 330)) (make-inspector insp1) make-arg ...)]
                         [(ctype cmake cpred csel cset) (make-struct-type 'c type 3 2 #f null (make-inspector insp1) make-arg ...)])
              (arity-test bmake 2 2)
              (arity-test bmakex 1 1)

              ;; Override ok:
              (make-struct-type 'bb type 0 0 #f (list (cons prop:p 12)) 'current make-arg ...)
              (make-struct-type 'bb btype 0 0 #f (list (cons prop:p3 12)) 'current make-arg ...)

              (err/rt-test (make-struct-type 'bb type 0 0 #f (list (cons prop:p 12) (cons prop:p 13)) 'current make-arg ...) exn:application:mismatch?)
              (err/rt-test (make-struct-type 'bb btype 0 0 #f (list (cons prop:p3 12) (cons prop:p3 13)) 'current make-arg ...) exn:application:mismatch?)
              (err/rt-test (make-struct-type 'bb #f 0 0 #f (list (cons prop:p 12) (cons prop:p2 12) (cons prop:p 13)) 'current make-arg ...) exn:application:mismatch?)
              (err/rt-test (make-struct-type 'bb type 0 0 #f (list (cons (let-values ([(p p? p-v)
                                                                           ...
                                                                           p)
                                                                         0))
                                             'current make-arg ...) exn:fail:contract:divide-by-zero?)
              ...
              (let ([a-b (bmake 'bone 'btwo)]
                ...
                (parameterize ([current-inspector insp1])
                  ...
                  (... '...)))))))))
  (make-struct-type-tests make-struct-type struct-type?)
  (let-values ([(struct:klass make-klass klass? klass-ref) (make-struct-metatype 'klass #f 2)])
    (define (ok-klass? v)
      (and (klass? v)
           (eq? (klass-ref v 0) 'one)
           (eq? (klass-ref v 1) 'two)))
    (make-struct-type-tests make-klass ok-klass? #f null #f #f 'one 'two)
    (let ()
      (define klass-get (make-struct-type-metaaccessor klass-ref))
      ...
      (err/rt-test (make-struct-field-accessor klass-ref 3 'get-klass-three))
      (test 'klass-one-ref object-name klass-one)
      (test 'klass-two-ref object-name klass-two)
      (test 'make-klass-struct-type object-name make-klass)
      (test #t struct-predicate-procedure? klass?)
      (test #f struct-constructor-procedure? make-klass)
      (test #t struct-accessor-procedure? klass-ref)
      (test #f struct-mutator-procedure? klass-ref)
      (test #t struct-type? struct:klass)
      (test #t struct-metatype? struct:klass)
      (err/rt-test (struct-type-info struct:klass) exn:fail:contract? #rx"inspector")
      (err/rt-test (struct-type-make-constructor struct:klass) exn:fail:contract? #rx"inspector")
      (err/rt-test (struct-type-make-predicate struct:klass) exn:fail:contract? #rx"inspector")
      (let ()
        ;; parent and child aren't required to have the same metaclass
        (define-values (s:a make-a a? a-ref a-set!) (make-struct-type 'a #f 1 0 #f null #f))
        (define-values (s:b make-b b? b-ref b-set!) (make-klass 'b s:a 2 0 #f null #f #f '(1) #f #f 'One 'Two))
        (define a-0 (make-struct-field-accessor a-ref 0))
        (define b-1 (make-struct-field-accessor b-ref 0))
        (define b-2 (make-struct-field-accessor b-ref 1))
        (test #t struct-accessor-procedure? a-0)
        (test #t struct-accessor-procedure? b-1)
        (test #t struct-accessor-procedure? b-2)
        (define a (make-a 'a0))
        (define b (make-b 'b0 'b1 'b2))
        (define-values (meta-a meta-a-skip?) (struct-info a))
        (define-values (meta-b meta-b-skip?) (struct-info b))
        (test #f meta-a-skip?)
        (test #f meta-b-skip?)
        (test #t struct-type? meta-a)
        (test #t struct-type? meta-b)
        (test #f struct-metatype? meta-a)
        (test #f struct-metatype? meta-b)
        (test #f klass? meta-a)
        (test #t klass? meta-b)
        (test 'no klass-get a 'no)
        (test s:b klass-get b 'no)
        (err/rt-test (make-struct-metatype 'klass2 s:a 1))
        (err/rt-test (make-struct-metatype 'klass2 s:b 1))
        (test 'One klass-one meta-b)
        (test 'Two klass-two meta-b)
        (test 'One klass-meta-one b #f)
        (test 'Two klass-meta-two b #f)
        (err/rt-test (klass-one meta-a))
        (test #f klass-meta-one a #f)
        (test #t a? a)
        (test #t a? b)
        (test #f b? a)
        (test 'a0 a-ref a 0)
        (test 'b0 a-ref b 0)
        (err/rt-test (a-ref b 1))
        (test 'b1 b-ref b 0)
        (test 'b2 b-ref b 1)
        (err/rt-test (b-ref b 2))
        (err/rt-test (b-ref a 0))
        (test 'a0 a-0 a)
        (test 'b0 a-0 b)
        (test 'b1 b-1 b)
        (test 'b2 b-2 b)
      ...
      (let ()
        ;; parent and child aren't required to have the same metaclass
        (define-values (s:a make-a a? a-ref a-set!) (make-klass 'a #f 2 0 #f null #f #f '(1) #f #f 'One 'Two))
        (define-values (s:b make-b b? b-ref b-set!) (make-struct-type 'b s:a 1 0 #f null #f))
        (define a-0 (make-struct-field-accessor a-ref 0))
        (define a-1 (make-struct-field-accessor a-ref 1))
        (define b-2 (make-struct-field-accessor b-ref 0))
        (test #t struct-accessor-procedure? a-0)
        (test #t struct-accessor-procedure? a-1)
        (test #t struct-accessor-procedure? b-2)
        (define a (make-a 'a0 'a1))
        (define b (make-b 'b0 'b1 'b2))
        (define-values (meta-a meta-a-skip?) (struct-info a))
        (define-values (meta-b meta-b-skip?) (struct-info b))
        (test #f meta-a-skip?)
        ...
        (test #t klass? meta-a)
        (test #f klass? meta-b)
        (test s:a klass-get a 'no)
        (test 'no klass-get b 'no)
        (err/rt-test (make-struct-metatype 'klass2 s:a 1))
        (err/rt-test (make-struct-metatype 'klass2 s:b 1))
        (test 'One klass-one meta-a)
        (test 'Two klass-two meta-a)
        (err/rt-test (klass-one meta-b))
        (test #t a? a)
        (test #t a? b)
        (test #f b? a)
        (test 'a0 a-ref a 0)
        ...
        (test 'b0 a-ref b 0)
        (test 'b1 a-ref b 1)
        (err/rt-test (a-ref b 2))
        (test 'b2 b-ref b 0)
        (err/rt-test (b-ref b 1))
        (err/rt-test (b-ref a 0))
        (test 'a0 a-0 a)
        (test 'a1 a-1 a)
        (test 'b0 a-0 b)
        ...
        (test 'b2 b-2 b)
        (err/rt-test (b-2 a)))
      (err/rt-test (make-struct-type 'a struct:klass 1 0 #f null #f))
      (err/rt-test (make-klass 'a #f 2 0 #f null 'prefab #f '(1) #f #f 'One 'Two))
      (let-values ([(struct:klass2 make-klass2 klass2? klass2-ref) (make-struct-metatype 'klass2 struct:klass 1)])
        (define (ok-klass2? v)
          (and (klass? v)
               (klass2? v)
               (eq? (klass-ref v 0) 'one)
               (eq? (klass-ref v 1) 'two)
               (eq? (klass2-ref v 0) 'three)))
        (make-struct-type-tests make-klass2 ok-klass2? #f null #f #f 'one 'two 'three)
        (let ()
          (define klass2-get (make-struct-type-metaaccessor klass2-ref))
          (define klass2-three (make-struct-field-accessor klass2-ref 0 'three-ref))
          (err/rt-test (make-struct-field-accessor klass2-ref 1 'get-klass-three))
          (test 'klass2-three-ref object-name klass2-three)
          (test 'make-klass2-struct-type object-name make-klass2)
          (test #t struct-predicate-procedure? klass2?)
          (test #f struct-constructor-procedure? make-klass2)
          (test #t struct-accessor-procedure? klass2-ref)
          (test #f struct-mutator-procedure? klass2-ref)
          (test #t struct-type? struct:klass2)
          (test #t struct-metatype? struct:klass2))))))
