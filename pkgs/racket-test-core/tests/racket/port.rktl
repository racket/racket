...
(for ([grapheme-on? (in-list '(#t #f))])
  (parameterize ([port-count-graphemes-enabled grapheme-on?])
    ;; Test provided by @wcs4217
    (let ([p (open-input-bytes #"\rx\ny")])
      (test #f port-counts-lines? p)
      (test #f port-counts-graphemes? p)
      (port-count-lines! p)
      (test #t port-counts-lines? p)
      (test grapheme-on? port-counts-graphemes? p)
    ...
    (let ()
      ...
      (define (grapheme-bump loc)
        (if grapheme-on?
            loc
            (list (car loc) (add1 (cadr loc)) (add1 (caddr loc)))))
      (define (grapheme-bump2 loc)
        (if grapheme-on?
            loc
            (list (car loc) (cadr loc) (add1 (caddr loc)))))

      (write-string "e\u300" o)
      (test (grapheme-bump '(3 2 9)) next-location o)
      ...
      (test (grapheme-bump '(3 2 9)) next-location i)  ; UTF-8 concluded, grapheme cluster is still tentative, though

      (write-string "!" o)
      (test (grapheme-bump '(3 3 10)) next-location o)
      (test #"!" read-bytes 1 i)
      (test (grapheme-bump '(3 3 10)) next-location i)

      (write-string "\r" o)
      (test (grapheme-bump2 '(4 0 11)) next-location o)
      (test #"\r" read-bytes 1 i)
      (test (grapheme-bump2 '(4 0 11)) next-location i)
      ...
      (write-string "." o)
      (test (grapheme-bump2 '(4 1 12)) next-location o)
      (test #"." read-bytes 1 i)
      (test (grapheme-bump2 '(4 1 12)) next-location i)

      (write-string "app\u03BBe" o)
      (test (grapheme-bump2 '(4 6 17)) next-location o)
      (test "app\u03BBe" read-string 5 i)
      (test (grapheme-bump2 '(4 6 17)) next-location i)
