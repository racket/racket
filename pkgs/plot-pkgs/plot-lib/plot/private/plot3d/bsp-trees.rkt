...
(: walk-bsp-trees (-> Symbol FlVector (HashTable Integer (Listof BSP-Shape))
                      (HashTable Integer (Listof (Listof BSP-Shape)))))
(define (walk-bsp-trees key view-dir detail-shapes)
  ...
  (for/hasheq : (HashTable Integer (Listof (Listof BSP-Shape))) ([layer  (in-list layers)])
    (define bsp (hash-ref bsp-trees layer (λ () (bsp-leaf empty))))
    (define ss (hash-ref detail-shapes layer (λ () empty)))
    
    (: in-order-ss (Listof (Listof BSP-Shape)))
    (define in-order-ss
      (let loop ([bsp  (bsp-tree-insert bsp ss)])
        (match bsp
          [(bsp-node plane ss left right)
           ...
           (cond [(cos-angle . > . 1e-16)
                  (append (loop left) (list ss) (loop right))]
                 [(cos-angle . < . -1e-16)
                  (append (loop right) (list ss) (loop left))]
                 [else
                  (append (loop left) (loop right) (list ss))])]
          [(bsp-leaf ss)
           (list ss)])))
