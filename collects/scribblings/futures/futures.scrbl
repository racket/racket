...
@defproc[(future [thunk (-> any)]) future?]{
  Starts running @scheme[thunk] in parallel.  The @scheme[future] procedure returns immediately with a future descriptor value.
}

@defproc[(touch [f future?]) any]{
  Returns the value computed in the future @scheme[f], blocking until the future completes (if it has not already completed).
}
...
@defproc[(processor-count) exact-positive-integer?]{
  Returns the number of processors/cores/hardware threads available on the current system.
}

@section[#:tag "besteffortpar"]{Best-Effort Parallelism}

The @scheme[future] API represents a best-effort attempt to execute an arbitrary segment of code in parallel.  When designing programs and algorithms which leverage @scheme[future] for parallel speedup, there are a number of performance considerations to be aware of. 
...
Though this program still produces the same output, no work is being done in parallel.  Because @scheme[printf] is considered an unsafe operation, f will block, and the print invocation (along with the subsequent add) will not be performed until the @scheme[touch] call.  

@section[#:tag "logging"]{How Do I Keep Those Cores Busy?}

Because it is not always obvious when or where unsafe operations may be causing unacceptable performance degradation in parallel programs, futures can be configured to generate trace output using the standard logging command-line switches.  This output can tell us which operations a given future is waiting on at a particular point during the program run.  For example, running the code in the previous example in the debug log level produces the following output: 
