  ...
  (define-syntax (>>P stx)
    (syntax-case stx ()
      [(>>P pr (constructor var ...) pattern . clauses)
       #'(>>Prim pr #t (constructor var ...) pattern pattern . clauses)]))

  ;; >>P with no restamping
  (define-syntax (>>Pn stx)
    (syntax-case stx ()
      [(>>Pn pr (constructor var ...) pattern . clauses)
       #'(>>Prim pr #f (constructor var ...) pattern pattern . clauses)]))

  (define-syntax (>>Prim stx)
    (syntax-case stx ()
      [(>>Prim pr restamp? cons+vars inp outp clauses)
       #'(>>Prim pr restamp? cons+vars inp outp clauses #:with values)]
      [(>>Prim pr restamp? cons+vars inp outp clauses #:with transform)
       #'(>>Prim pr restamp? cons+vars inp outp clauses
                 #:with2 (lambda (pr stx) (values pr (transform stx))))]
      [(>>Prim pr restamp? (constructor var ...)
               in-pattern
               out-pattern
           ...
           #`(let ([prule-var pr])
               (let-values ([(fill/bind s-tmp)
                 ...
                 (let ([new-e2 (if (or (interrupted-wrap? prule-var) (error-wrap? prule-var))
                                   #f
                                   (with-syntax ([in-pattern (deriv-e1 prule-var)])
