...
;; also use a unique value to determine the end of the sequence instead of using #f
(define-syntax lambda-generator
  (syntax-rules ()
    [(_ (args ...) body0 bodies ...)
     (lambda (args ...)
       (let* ([last (lambda () (void))]
              ;; current is a function that invokes user code and produces values
              [current (lambda ()
              ...
              [seq (make-do-sequence (lambda ()
                                       (values
                                         ...
                                         (lambda (i) (let-values ([(value next) (current)])
