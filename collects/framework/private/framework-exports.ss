  ...
  (syntax-case stx ()
    [(_ id)
     #'(id
        (text:autocomplete-append-after
         ...
         @{A string that is inserted after a completions is inserted by a
             @scheme[text:autocomplete]
             instance.
             
             Defaults to @scheme[""].})
        (text:autocomplete-limit
         (parameter/c (and/c integer?
                             exact?
                             positive?))
         ()
	   ...
	   Makes a number snip that shows the decimal expansion for @scheme[number]
		 The boolean indicates if a @tt{#e} prefix appears
		 on the number.
		 
		 See also
		 @scheme[number-snip:make-fraction-snip].})
	 ...
	 @{
         ...
         See also
         @scheme[number-snip:make-repeating-decimal-snip].})
        (version:add-spec
         (any/c any/c . -> . void?)
         (spec revision)
	 @{
	 ...
	 @{
         ...
         is used in the help menu (see @scheme[frame:standard-menus%]) and in
         frame titles (see @scheme[frame:editor%]).
         
         The first case in the case-lambda returns
         the current name, and
         the second case in the case-lambda sets
         the name of the application to @scheme[name].})
	 ...
	 @{
	   ...
	   @scheme[f] with name @scheme[labels] to the preferences dialog
	   box.
	   ...
	   If multiple calls to @scheme[preferences:add-preference-panel]
	   pass the same prefix of strings, those panels are placed in the
	   same children.
	   
	   When the preference dialog is opened for the first
	   time, the function @scheme[f] is called with a panel, and
	   @scheme[f] is expected to add a new child panel to it and add
	   whatever preferences configuration controls it wants to that
	   panel. Then, @scheme[f]'s should return the panel it added.})
	 ...
	 @{
	   Adds a preferences panel for configuring
        ...
        (preferences:add-to-warnings-checkbox-panel
         ...
         (proc)
	 @{
	   Saves @scheme[proc] until the preferences panel is
	 ...
	 @{
	   ...
	   Hides the preferences dialog.})
        (preferences:add-on-close-dialog-callback
         ...
         (cb)
	 @{
	   Registers @scheme[cb]. Next time the
		     user clicks the OK button the preferences
		     dialog, all of the @scheme[cb] functions
		     are called, assuming that each of the callbacks
		     passed to
		     @scheme[preferences:add-can-close-dialog-callback]
		     succeed.})
	 ...
	 @{
	   Registers @scheme[cb]. Next time the
		     user clicks the OK button the preferences
		     dialog, all of the @scheme[cb] functions
		     are called. If any of them return @scheme[#f],
		     the dialog is not closed.
		     
		     See also
		     @scheme[preferences:add-on-close-dialog-callback].})
	 ...
	 @{
         ...
         to autosave, the @scheme[do-autosave] method of the object is
	 ...
	 @{
	   ...
	   @scheme[exit:exiting?]
	   function to @scheme[#t].}
	   @item{invokes the exit-callbacks, with 
	   @scheme[exit:can-exit?]
	   If none of the ``can?'' callbacks return @scheme[#f],}
	   @item{
	   invokes
	   @scheme[exit:on-exit]
	   and then}
	   ...
	   @scheme[exit] (a mzscheme procedure)
	   and (if @scheme[exit] returns) sets the
	   result of
	   @scheme[exit:exiting?]
	   back to @scheme[#t].}}})
        (exit:user-oks-exit
	 ...
	 @{
	   This parameter determines the parent of the dialogs created by
		...
		@scheme[finder:common-get-file],
		@scheme[finder:common-put-file],
		@scheme[finder:common-get-file-list],
		@scheme[finder:std-get-file], and
		  @scheme[finder:std-put-file].})
	 ...
	 @{
         ...
         @scheme[default-extension] argument to
         @scheme[put-file].
         
         Its default value is @scheme[""].})
        (finder:default-filters
         (parameter/c (listof (list/c string?
				      string?)))
	 ()
	 @{
	   This parameter controls the default extension for the framework's 
		@scheme[finder:put-file]
		dialog. Its value gets passed as the
		@scheme[default-filters] argument to
		@scheme[put-file].
	 ...
	 @{
	   Queries the user for a filename.
		   ...
		   If the result of
		   @scheme[(preferences:get 'framework:file-dialogs)]
		   is @scheme['std] this calls 
		   @scheme[finder:std-put-file], 
		   and if it is @scheme['common],
		   @scheme[finder:common-put-file]
		   is called.})
        (finder:get-file
         (->* ()
	   ...
	   Initializes a preference for the
        ...
        (frame:add-snip-menu-items
         ...
         ((menu menu-item%) 
          ((func void)))
	 @{
	   Inserts three menu items into @scheme[menu],
		   ...
		   Uses @scheme[menu-item%] as the class for
		   the menu items.
	 ...
	 @{
	   This function inserts a format handler.
		...
		@scheme[handler:find-named-format-handler].
		If @scheme[pred] is a string, it is matched with
		the extension of a filename by
		@scheme[handler:find-format-handler].
		If @scheme[pred] is a list of strings,
		they are each matched with the extension of a filename by
		@scheme[handler:find-format-handler].
		If it is a function, the filename is applied
		to the function and the
		functions result determines if this is the handler to use.
		
		The most recently added format handler takes
		precedence over all other format handlers.})
	 ...
	 @{
	   This function creates a frame or re-uses an existing frame to edit a file. 
		...
		and @scheme[(send (group:get-the-frame-group) get-open-here-frame)]
		returns a frame, the 
		@(method frame:open-here<%> open-here)
		...
		(see @scheme[handler:insert-format-handler]).
						       
	@itemize{
	@item{If @scheme[filename] is a string, this function checks the result of
	@scheme[group:get-the-frame-group]
	to see if the @scheme[filename] is already open by a frame in the
	group.
	@itemize{
	@item{If so, it returns the frame.}
	@item{If not, this function calls 
	@scheme[handler:find-format-handler]
	with @scheme[filename].
	@itemize{
	@item{
	If a handler is found, it is applied to
	@scheme[filename] and it's result is the final result. }
	@item{
	If not, @scheme[make-default] is used.}}}}}
	@item{
	If @scheme[filename] is @scheme[#f], @scheme[make-default] is used.}}})
	 ...
	 @{
	   This returns the autowrap's 
		...
		The bitmap may not respond @scheme[#t] to the
		@method[bitmap% ok?]
		method.})
	 ...
	 @{
	   This function returns a 
		...
		The cursor may not respond @scheme[#t] to the
		@link cursor ok?
		method.})
	 ...
	 @{
	   Chains the keymap defined by @scheme[user-keybindings-path-or-require-spec] to 
		  the global keymap, returned by 
		  @scheme[keymap:get-global].
		  
		  If @scheme[user-keybindings-path-or-require-spec] is a path, the module is loaded
		  directly from that path. Otherwise, @scheme[user-keybindings-path-or-require-spec]
		  is treated like an argument to @scheme[require].})
        (keymap:add-to-right-button-menu
	 (parameter/c (-> (is-a?/c popup-menu%) (is-a?/c editor<%>) (is-a?/c event%) void?))
	 ()
	 @{
	   When the keymap that 
		...
		function
		@scheme[append-editor-operation-menu-items]
		is called.
		
		See also
		@scheme[keymap:add-to-right-button-menu/before].})
	 ...
	 @{
	   When the keymap that 
		...
		See also
		@scheme[keymap:add-to-right-button-menu].})
	 ...
	 @{
	   Thus function parameterizes the call to @scheme[thunk-proc] by
		 setting the keymap-initialization procedure
		 (see @scheme[current-text-keymap-initializer])
	 ...
	 @{
	   Returns a string that denotes the same keybindings as the input
		   ...
		   strings can be compared with @scheme[string=?].})
	 ...
	 @{
	   This returns a keymap for handling standard editing operations.  It
		...
		where each key is prefixed with the menu-shortcut key,
		based on the platform. Under unix,
		the shortcut is @scheme["a:"]; under windows the
		shortcut key is @scheme["c:"] and under MacOS,
		the shortcut key is
		@scheme["d:"].})
	 ...
	 @{
	   This prefixes a key with all of the different meta prefixes and
		...
		@scheme[(keymap:send-map-function-meta keymap "a" func)]
		maps @scheme["m:a"] and @scheme["ESC;a"] to @scheme[func].})
	 ...
	 @{
	   @index{Meta}
	   Most keyboard and mouse mappings are inserted
	   into a keymap by calling
	   the keymap's @(method keymap% map-function) method.
	   However, ``meta'' combinations
	   require special attention. The @scheme["m:"] prefix recognized by
	   @(method keymap% map-function)
	   applies only to the Meta key that exists on
	 ...
	 @{
	   This extends a 
		...
		@itemize{
		@item{@mapdesc[ring-bell any] --- Rings the bell (using @scheme[bell]) and
		removes the search panel from the frame, if there.}
		@item{ @mapdesc[save-file key] --- Saves the buffer. If the buffer has 
		no name, then @scheme[finder:put-file]@index["finder:put-file"] is invoked.}
		@item{ @mapdesc[save-file-as key] --- Calls
			       @scheme[finder:put-file]@index["finder:put-file"]
			       to save the buffer.}
		@item{ @mapdesc[load-file key] --- Invokes @scheme[finder:open-file]@index["finder:open-file"].}
	 ...
	 @{
	   Removes @scheme[keymap] from the keymaps chained to @scheme[editor].
		   Also (indirectly) removes all keymaps chained to @scheme[keymap] from @scheme[editor],
		   since they are removed when unchaining @scheme[keymap] itself.
		   
		   Each of the keymaps chained to @scheme[editor] must be an
		   @scheme[keymap:aug-keymap%]
		   and @scheme[keymap] cannot be the result of
		   @scheme[(send editor get-keymap)]
		   That is, @scheme[keymap] must be chained to some keymap attached
		   to the editor.})
        ...
        (scheme:text-balanced?
         ...
         ((text)
          ((start 0) (end #f)))
	 @{
	   Determines if the range in the editor from @scheme[start] to @scheme[end] in @scheme[text]
		      is a matched set of parenthesis. If @scheme[end] is @scheme[#f], it
		      defaults to the last position of the @scheme[text].
	 ...
	 @{
	   Returns a table mapping from symbols (naming the categories that
							...
							@scheme[scheme:short-sym->pref-name]
							and
							@scheme[scheme:short-sym->style-name]
							.
							
							See also
							@scheme[scheme:get-white-on-black-color-prefs-table].})
	 ...
	 @{
	   Returns a table mapping from symbols (naming the categories that
							...
							See also
							@scheme[scheme:get-color-prefs-table].})
	 ...
	 @{
	   The name of the style
	       (in the list returned by @scheme[editor:get-standard-style-list])
	       that holds the default color.})
        (editor:set-standard-style-list-delta 
         (string? (is-a?/c style-delta%) . -> . void?)
         (name delta)
	 @{
	   Finds (or creates) the style named by @scheme[name] in
		 the result of 
		 @scheme[editor:get-standard-style-list]
		 and sets its delta to @scheme[delta].
		 
		 If the style named by @scheme[name] is already in
	 ...
	 @{
	   This function registers a color preference and initializes the
		...
		and 
		@scheme[preferences:set-un/marshall ]
		to install the pref for @scheme[pref-name], using
		@scheme[color/sd] as the default color. The preference
		is bound to a @scheme[style-delta%],
		and initially the @scheme[style-delta%]
		changes the foreground color to @scheme[color/sd], unless @scheme[color/sd] is a style
		delta already, in which case it is just used directly.
		Then, it calls 
		@scheme[editor:set-standard-style-list-delta]
		passing the @scheme[style-name] and the current value
		of the preference @scheme[pref-name].
		
		Finally, it adds calls
		@scheme[preferences:add-callback ]
		to set a callback for @scheme[pref-name] that
		updates the style list when the preference changes.
		
		If @scheme[white-on-black-color] is not @scheme[#f], then the color of the
		@scheme[color/sd] argument is used in combination with @scheme[white-on-black-color]
	 ...
	 @{
	   Adds a preferences panel that configures the background
        ...
        (color-prefs:add-to-preferences-panel
         ...
         (name func)
	 @{
	   Calls @scheme[func] with the subpanel of the preferences coloring panel that
		 corresponds to @scheme[name].})
	 ...
	 @{
	   Builds a panel with a number of controls for configuring
		  ...
		  The @scheme[parent] argument specifies where the panel will be
		  placed. The @scheme[pref-sym] should be a preference
		  (suitable for use with @scheme[preferences:get] and @scheme[preferences:set]).
		  The @scheme[style-name] specifies the name of a style in the
		  style list returned from
		  @scheme[editor:get-standard-style-list]
		  and @scheme[example-text] is shown in the panel
		  so users can see
...
(define-syntax (make-framework-exports/srcloc-preserved stx)
  (let ()
    ...
    (syntax-case stx ()
      [(macro-name arg ...)
       (with-syntax ([framework-exports/srcloc-preserved (datum->syntax #'macro-name 'framework-exports/srcloc-preserved)])
         (let ([marshalled
                (let loop ([stx #'(arg ...)])
                  (cond
